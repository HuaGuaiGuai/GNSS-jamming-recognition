function BLGNISpectrogramGen(jsr_range,f_range,f_num,fs,T,gnss_signal,P_s_dBW,noise,root_path,subfile_path,num_per_type,res)
img_index = 1;
    t = 0:1/fs:T-(1/fs);
    for j = 1:f_num  
        close all;
        ...
        for jsr = jsr_range(1):jsr_range(2)
            ...
            title('');
            colorbar('off');
            image_name = sprintf('%s%simage%05d_%02d%s_jsr%02d.png',root_path,subfile_path,img_index,11,'BLGNI',jsr);
            exportgraphics(fig,image_name,'Resolution',res);
            img_index = img_index + 1;
            close all;
            if img_index > num_per_type
                close all;
                fprintf('Generate %d sheet of BLGNI spectrogram\r',img_index-1);
                return;
            end
        end
    end
...
function jamming = BLGNIGen(power,BW_NB_Hz,fs,T)
